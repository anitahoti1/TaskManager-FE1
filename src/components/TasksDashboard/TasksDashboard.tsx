...
// const names = [{name:"rina<3",age:"25"}, {name:"anita<3",age:"35"}];
  ...
  const handleDeleteTask = (id: number) => {
    ...
    setTasksFromBack({
      toDo: tasksFromBack.toDo.filter(t => t.id !== id),
      inprogress: tasksFromBack.inprogress.filter(t => t.id !== id),
      review: tasksFromBack.review.filter(t => t.id !== id),
      done: tasksFromBack.done.filter(t => t.id !== id),
    });
      ...
      {taskBeingEdited && (
        <EditTaskModal
          ...
          onSave={(updatedTask) => {
            ...
            const updateTaskInArray = (arr: ITask[]) =>
              arr.map(t => t.id === updatedTask.id ? updatedTask : t);

            setTasksFromBack({
              toDo: updateTaskInArray(tasksFromBack.toDo),
              inprogress: updateTaskInArray(tasksFromBack.inprogress),
              review: updateTaskInArray(tasksFromBack.review),
              done: updateTaskInArray(tasksFromBack.done),
            });
